...
**当用户要求生成文章时，主动引导用户选择配置：**

### 引导话术模板

```
📝 好的，我来帮你生成文章！请先确认以下配置：
...
| 配置项 | 选项 | 默认值 |
|--------|------|--------|
| 主题色 | default/roseGold/jadeGreen/vibrantOrange | roseGold |
| 文章长度 | concise/standard/deep | standard |
| 配图 | full/cover/none | full |
| 风格 | professional/casual/educational | professional |

### 快速配置示例

- `默认` → 熏衣紫 + 标准版 + AI配图
- `经典蓝 深度版` → 经典蓝 + 深度分析 + AI配图
- `翡翠绿 精简 仅封面` → 翡翠绿 + 精简版 + 仅封面
- `技术文章` → 经典蓝 + 标准版 + AI配图
...
| 主题 | 颜色代码 | 适用场景 |
|------|---------|---------|
| default | `#0F4C81` 经典蓝 | 技术教程、开发文档 |
| roseGold | `#8B7BA8` 熏衣紫 | 优雅专业、通用文章 |
| jadeGreen | `#009874` 翡翠绿 | 自然健康、生活类 |
| vibrantOrange | `#FA5151` 活力橘 | 热点评论、醒目文章 |
